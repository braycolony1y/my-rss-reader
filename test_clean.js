...
const trimArticleMarkupAtSemanticBoundary = function(markup) { return markup; }
...
const ARTICLE_CACHE_VERSION = 12;
...
async function getCachedArticle(url) {
    ...
    try {
        ...
        if (cached.version !== ARTICLE_CACHE_VERSION || !cached.cachedAt || Date.now() - cached.cachedAt >= ARTICLE_CACHE_TTL_MS || !cached.result?.content) {
...
const env = {
    RSS_DATA: {
        ...
        put: (key, value, options = {}) => withDbLock(async () => {
            ...
            if (['feeds', 'articles', 'smartRawArticles', 'smartClusters'].includes(key)) {
...
app.get('/api/data', authMiddleware, async (req, res) => {
    ...
    if (filterType === 'smart') {
        ...
        if (hideRead) filteredArticles = filteredArticles.filter(article => !readStates.includes(article.link));
...
const trimArticleMarkupAtSemanticBoundary = function(markup) {
...
const html = `<div class="embedded-suggested-carousel">
    <a href="#" class="embedded-suggested-card" target="_blank">
        <img src="#">
        <div class="embedded-suggested-content">...</div>
    </a>
</div>`;
