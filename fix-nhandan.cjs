const fs = require('fs');
const path = './src/sources/NhanDanSource.js';
let content = fs.readFileSync(path, 'utf8');

const regex = /\/\/ Clean up some noise[\s\S]*?return articleHtml;/;

const replacement = `// Clean up some noise
        articleHtml = articleHtml.replace(/<div\\b[^>]*class=["'][^"']*box-author[^"']*["'][^>]*>[\\s\\S]*?<\\/div>/gi, '');
        ...
        articleHtml = articleHtml.replace(/<div\\b[^>]*class=["'][^"']*widget-related[^"']*["'][^>]*>([\\s\\S]*?)<\\/div>\\s*<\\/div>/gi, (m, inner) => {
        ...
        const authorMatch = html.match(/<meta\\s+name=["']author["']\\s+content=["']([^"']+)["']/i) || 
                            html.match(/<div\\b[^>]*class=["'][^"']*box-author[^"']*["'][^>]*>\\s*<strong>([^<]+)<\\/strong>/i);
...
<div class="embedded-suggested-articles">
    ...
    <div class="embedded-suggested-carousel">
        ...
        const relatedMatch = html.match(/<div\\b[^>]*class=["'][^"']*related-news[^"']*["'][^>]*>([\\s\\S]*?)<\\/ul>\\s*<\\/div>/i);
        if (relatedMatch) {
...
console.log("Updated NhanDanSource.js");
