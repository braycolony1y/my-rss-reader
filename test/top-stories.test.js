...
test('source memberships are hard eligibility boundaries independent of language',()=>{
...
assert.deepEqual(
    allowedDestinations(
        {...a,feedUrl:'https://unknown.com/rss'},
        sources
    ),
    []
);
...
const stories=Array.from(
    ...
    (_,i)=>article(
        `source${i}`,
        {
            title:`Critical vulnerability in product ${i} confirmed`,
            editorialAssessment:{
                ...
                confidence:.9,
                exclude:false,
                reason:'Material security event'
            }
        }
    )
);
...
assert.ok(
    ranked.every(
        a=>
            Number.isFinite(a.ranking.score) &&
            a.topStory.rank>0
    )
);
...
const minor=article(
    'minor',
    {
        title:'Purple keyboard review',
        content:'A new keyboard color.',
        editorialAssessment:{
            policyVersion:'ai-editorial-v1',
            revision:'minor',
            eligibleDestinations:[
            ...
            confidence:.9,
            exclude:false,
            reason:'Routine low-impact product story'
        }
    }
);
...
test('AI editorial assessment chooses the eligible domestic subject feed',async()=>{
const {index}=harness();
...
const a=article(
    ...
    {
        language:'vi',
        title:'Ngân hàng Nhà nước Việt Nam điều chỉnh lãi suất',
        smartCategory:'news_vietnam',

        editorialAssessment:{
            policyVersion:'ai-editorial-v1',
            revision:'sbv-finance',
            eligibleDestinations:[
                'news_vietnam',
                'finance_vietnam'
            ],
            destination:'finance_vietnam',
            relevance:1,
            impact:.8,
            novelty:.8,
            confidence:.95,
            exclude:false,
            reason:'Vietnam monetary policy decision'
        }
    }
);

const result=await index.rank(
    [a],
    [
        {
            url:a.feedUrl,
...
assert.equal(
    result[0].topStory.feed,
    'finance_vietnam'
);

assert.equal(
    result[0].ranking.signals.relevance,
    1
);
});
