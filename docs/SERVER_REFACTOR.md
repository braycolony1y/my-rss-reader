...
Article cache version remains **54**, with seven-day normal reads and 14-day last-known retention cleanup. Read Later and all Board folders protect snapshots, archive history, and PDFs without expiration, including deleted sources. Dedicated archives expire 14 days after leaving both Board and Read Later; unprotected deleted snapshots expire 14 days after detection. PDF exports fetch up to ten pages concurrently and report live progress. Ordinary article deletion still requires independent reader confirmation; VOZ preserves page-specific archives. Strict per-source methods, challenge detection, reader fallback order, response-body handling, authentication, status codes and database string/JSON formats are unchanged. No dependency was added.
