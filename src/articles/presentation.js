...
export function createArticlePresentation({
    ...
    const storyPrewarmBucket = article => {
        ...
        return `${vietnam ? 'vietnam' : 'global'}:${section}`;
    };
    ...
    // TOP_STORIES_GLOBAL_CANONICAL_V1
    function canonicalSmartCategory(value) {
        return String(value || '')
            .replace(/_world$/, '_global')
            .replace(/_foreign$/, '_global');
    }

    function canonicalTopStoryFeed(value) {
        return canonicalSmartCategory(value);
    }
    ...
    function smartTechRegion(article) {
        ...
        if (['foreign', 'world', 'global'].includes(region)) {
            return 'global';
        }
    ...
    function smartArticleMatchesSection(
        ...
        const category =
            canonicalSmartCategory(
                article?.smartCategory
            );

        if (filterValue === 'news') {
            return category === `news_${smartRegion}`;
        }

        if (filterValue === 'finance') {
            return category === `finance_${smartRegion}`;
        }
        ...
        return (
            category ===
            canonicalSmartCategory(filterValue)
        );
    }
    ...
    async function prioritizeVisibleBriefings({
        ...
        if (!briefings.setViewport(viewKey, ids, generation)) {
            return { success: true, staleViewport: true, promoted: 0 };
        }
        let promoted = 0;
        const updates = [];
        ...
        for (
            ...
            const briefing = await briefings.get(
                article,
                article.topStory.feed,
                {
                    priority: 2,
            ...
            updates.push({ clusterId: id, materialVersion: article.topStory.material_version, briefing });
            promoted++;
        }
    ...
    async function serveSmartData(req, res) {
        ...
        if (isTop) {
            ...
            // TOP_STORIES_ONE_DESTINATION_V1
            const destination =
                filterValue === 'tech'
                    ? `tech_${smartRegion}`
                    : (
                        ['news', 'finance'].includes(filterValue)
                            ? `${filterValue}_${smartRegion}`
                            : canonicalSmartCategory(filterValue)
                    );

            filteredArticles = (snapshot?.articles || [])
                    .filter(
                        article =>
                            canonicalTopStoryFeed(
                                article?.topStory?.feed
                            ) === destination
                    );
        ...
        if (req.query.briefingRefresh !== '1') briefings.setActiveView(briefingViewKey);
